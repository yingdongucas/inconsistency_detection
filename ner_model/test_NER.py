...
def test_ner():
    ...
    model_name = get_ner_model_from_dir()
    ...
    for category in test_cat_list:
        ...
        SMALL_DATA_NAME = category + test_flg + config.ner_data_suffix
        ...
        SMALL_DATA_PD_GAZE = SMALL_DATA_PD_NOGAZE.replace(config.ner_data_suffix, '_gaze' + config.ner_data_suffix)
        ...
        if not gazetteer:
            ...
            model.load_params(save_path + model_name)
            # model_bkp = deepcopy(model)
            logging.info('finish loading, predicting starts...')

            py, py_score = model.predict(tx, tm, twx, tcm, tgaze=None, reload_model_path=save_path + model_name)
        ...
        str_write = utils.format_str(
            [category, float(acc), metric_result['vulnerable_version'][3], metric_result['vulnerable_version'][4],
             metric_result['vulnerable_software'][3], metric_result['vulnerable_software'][4]])
